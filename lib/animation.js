 ...
 */
Animation.__staticrules = function (el, property, duration) {
    var style = window.getComputedStyle(el),
        rule = style['transition'] || style['-o-transition'] ||
            style['-webkit-transition'];

    var cssduration, cssproperty;
    var subrules = rule.split(', ').filter(function (subrule) {
        var prop = subrule.split(' ')[0];
        return prop !== property && prop !== 'all';
    }, this).join(' ');

    if (duration === undefined) duration = cssduration;
...
Animation.prototype = {
    ...
    __init: function (el, property, duration) {
        ...
        this._staticrules = Animation.__staticrules(el, property, duration);
    ...
    __check: function (value, interrupted) {
        ...
        if (finished) {
            this.transition = null; // -> fire 'animationWillEnd'
        }
        else {
            this._next = next;
    ...
    css: function (transition) {
        ...
        var rule = this.property + ' ' + transition.css();
        if (this._staticrules) rule += ', ' + this._staticrules;
        return rule;
    },
