...
function Animation() {
    ...
    this._checkpoint = new Animation.Checkpoint(this);
    this._delegate = null;
}
...
Animation.prototype = {
    ...
    __delegate: function(type, arg0) {
        if (typeof this._delegate[type] === 'function') {
            this._delegate[type](this, arg0);
        }
    },

    set delegate(delegate) {
        assert_method(delegate, 'animationReachedCheckpoint');
    ...
    __check: function (value, interrupted) {
        ...
        var checkpoint = this._checkpoint.__check(value);

        this.__delegate('animationReachedCheckpoint', checkpoint);

        var subtransition = checkpoint.__apply();
        ...
        if (!subtransition) {
            ...
            var transition = this.css(subtransition);
            this.el.style.transition = transition;
            this.el.style.OTransition = transition;
    ...
    transitionend: function (evt) {
        this.__check(this._checkpoint.next, false);
    },
...
Animation.Checkpoint = function (animation) {
...
Animation.Checkpoint.prototype = {
    ...
    __apply: function () {
        ...
        // next must be a number unless animation's done
        assert(this.isFinish || is_number(this.next), "`next` is not a number");

        assert(is_ordered(this.value, this.next, this.dest),
               "Can't go from " + this.value + " through " + this.next +
               " to " + this.dest + "; `next` is improperly set");

        return this.isFinish ? null : this.animation.transition.slice(
            this.value, this.next
        );
    },

    toString: function () {return "[object Animation.Checkpoint]";}
};
...
//     /**
//      * Unless checkpoint.isFinish it *must* set checkpoint.next appropriately
//      */
