...
Animation.ScrollDelegate.prototype = {
    ...
    stateByValue: function (value) {
        var state = this._stepper.stateByValue.call(this, value),
            rstate = Math.round(state),
            e = rstate - state;
        if (e && e < 1e-3 && e > -1e-3) {
            console.warn("state within error to integer value: "  + state);
            // is the viewport scaled?
            state = rstate;
        }
        return state;
    },
    ...
    animationReachedPoint: function (animation, value) {
        ...
        // Then again: lib's client might actually *want* to
        // listen to "changeState" even if no redrawing is needed
        if (destState > state) {
    ...
    get range() {
        ...
        // TODO do you really need brute force here?
        var min = Infinity, max = -Infinity;
        for (var i = 0; i < numtiles; i++) {
            var pos = pos0 + i + (shift > i ? numtiles : 0);
            if (pos < min) min = pos;
            if (pos > max) max = pos;
        }
        return [min, max + 1];
    ...
    tileAtPosition: function (position) {
        var state = this._state;
        ...
        for (var i = 0; i < numtiles; i++) {
            var pos = pos0 + i + (shift > i ? numtiles : 0);
            if (pos === position) return this.tiles[i];
        }
        return null;
    },
    ...
    /**
     * @param {Number} dest
     * @return {Bool} Whether scrolling has (visual) effect
     */
    scrollTo: function (dest) {
        dest = Math.min(dest, this.maxForward);
        dest = Math.max(dest, -this.maxBackward);
        var animationDest = this.valueByState(dest),
            change = this._animation.dest !== animationDest;
        // ensure animation *is* running
        if (change || !this._animation._transition)
            this._animation.runTo(animationDest);
        return change;
    },
    ...
    tile: function (vpos) {
        ...
        if (vpos < this.FORWARD_BUFFER || vpos >= (len + this.FORWARD_BUFFER))
            return null;
        return this.tiles[(-this.dest + vpos) % len];
    },
